...
*   TeamRedMiner v0.7.11 ('ethash' additions: printing share diff in GH (always enabled), hashrate watchdog by option '--eth_hashwatch', better debug support with new options '--long_timestamps', '--pool_debug', added DAG allocation patch for certain mobo/bios combinations '--eth_dag_alloc_patch', fixed broken argon2/chukwa)
